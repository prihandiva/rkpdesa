        ...
        <div class="row align-items-center">
            <div class="col-md-12">
                <div class="page-header-title">
                    <h5 class="m-b-10 fw-bold text-dark">Edit RT</h5>
                </div>
                <ul class="breadcrumb">
    ...
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center py-3">
                <h6 class="mb-0 fw-bold text-primary"><i class="feather-edit-2 me-2"></i>Form Edit RT</h6>
                <div class="d-flex gap-2">
                    <a href="{{ route('rt.show', $rt->id_rt) }}" class="btn btn-sm btn-outline-info">
                        ...
                        <select class="form-select @error('id_dusun') is-invalid @enderror"
                            ...
                            @foreach($dusun as $d)
                                <option value="{{ $d->id_dusun }}"
                                    {{ old('id_dusun', $rt->id_dusun) == $d->id_dusun ? 'selected' : '' }}>
                                    {{ $d->nama }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select class="form-select @error('id_rw') is-invalid @enderror"
                            ...
                            @foreach($rws as $rw)
                                <option value="{{ $rw->id_rw }}"
                                        ...
                                        {{ old('id_rw', $rt->id_rw) == $rw->id_rw ? 'selected' : '' }}>
                                    {{ $rw->nama_rw }}
...
<script>
    ...
    const savedRwValue  = '{{ old('id_rw', $rt->id_rw) }}';
    ...
    function filterRW() {
        ...
        // Restore nilai
        if (savedRwValue) rwSelect.value = savedRwValue;
    }

    dusunSelect.addEventListener('change', function () {
        rwSelect.value = ''; // reset RW ketika Dusun berubah
        filterRW();
    });
